...
function shallowRender(elementOrFunction, context) {
  ...
  var element = withContext(context, function () {
    return TestUtils.isElement(elementOrFunction) ? elementOrFunction : elementOrFunction();
  });
...
function SkinDeep(getCurrentNode, renderer, instance) {
  var api = {
    reRender: function reRender(elementOrFunction, context) {
      if (renderer) {
    ...
    getMountedInstance: function getMountedInstance() {
      if (instance) return instance;
      throw new Error('This tree has no mounted instance');
    },
    subTree: function subTree(query, predicate) {
      var node = _findNode(getCurrentNode(), createFinder(query, predicate));
      return node && skinDeepNode(node);
    },
    subTreeLike: function subTreeLike(query, predicate) {
      var node = _findNode(getCurrentNode(), createFinder(query, predicate, 'isLike'));
      return node && skinDeepNode(node);
    },
    everySubTree: function everySubTree(query, predicate) {
      var finder = createFinder(query, predicate);
      return findNodes(getCurrentNode(), finder).map(skinDeepNode);
    },
    everySubTreeLike: function everySubTreeLike(query, predicate) {
      var finder = createFinder(query, predicate, 'isLike');
      return findNodes(getCurrentNode(), finder).map(skinDeepNode);
    },
    dive: function dive(paths, context) {
      var tree = api;
      while (paths.length) {
        ...
        tree = shallowRender(function () {
          return React.createElement(node.type, node.props);
        }, context);
      }
      return tree;
    },
    findNode: function findNode(query) {
      return _findNode(getCurrentNode(), createNodePredicate(query));
    },
    textIn: function textIn(query) {
      var node = _findNode(getCurrentNode(), createNodePredicate(query));
      return getTextFromNode(node);
    },
    text: function text() {
      return getTextFromNode(getCurrentNode());
    },
    fillField: function fillField(query, value) {
      var node = _findNode(getCurrentNode(), createNodePredicate(query));
    ...
    findComponent: function findComponent(type, props) {
      if (arguments.length == 1) {
        console.warn("Using a component in findComponent is deprecated. " + "Pass name and props as separate arguments instead");
        var search = type;
        type = getComponentName(search.type) || search.type;
        props = search.props;
      }
      return _findNode(getCurrentNode(), createFinder(type, props));
    },
    findComponentLike: function findComponentLike(type, props) {
      if (arguments.length == 1) {
        console.warn("Using a component in findComponent is deprecated. " + "Pass name and props as separate arguments instead");
        var search = type;
        type = getComponentName(search.type) || search.type;
      ...
      props = props || {};
      return _findNode(getCurrentNode(), function (node) {
        return matchComponentType(type, node) && subset(node.props, props);
      });
    },
...
function createNodePredicate(query) {
  ...
  if (query.match(/^[a-z][\w\-]*$/)) {
    // tagnames begin with lowercase
    return function (n) {
      return n.type == query;
    };
  }
  ...
  return function (n) {
    return n.type && getComponentName(n.type) == query;
  };
}
...
function createFinder(selector, predicate, isLike) {
  ...
  if (typeof predicate === 'object') {
    ...
    otherMatcher = function (node) {
      return isLike ? subset(node.props, predicate) : subset(node.props, predicate) && subset(predicate, node.props);
    };
  }
...
function _findNode(node, predicate) {
...
function getTextFromNode(_x) {
  ...
  _function: while (_again) {
    ...
    if (node.props && 'children' in node.props) {
      _x = childrenArray(node.props.children);
      _again = true;
      continue _function;
    }
